...
- **Score** (0.0--1.0): An absolute quality measurement for that dimension. Scores are not relative to other images -- a 0.7 means the same thing regardless of what else has been analyzed.
- **Tags**: Contextual labels with confidence values (0.0--1.0) describing specific properties detected in the image. Tags are emitted only when their confidence exceeds the configured threshold (default 0.25).
...
| Negative space | 0.15 | Distribution of low-edge-density blocks (sweet spot: 30--60% "empty") |
| Symmetry | 0.10 | Bilateral symmetry via horizontal flip-and-compare on edge map (IoU) |
| Diagonal dominance | 0.10 | Proportion of line energy from segments in the 30--60 degree band |
...
Saliency is approximated via edge density (Canny edges + box filter + center-bias Gaussian) rather than a learned model. This works well for anime's clean line art -- high-edge subjects stand out against low-edge backgrounds.
...
| 0.4--0.7 | Moderate composition -- functional framing without strong structural design |
...
| Color temperature | 0.05 | Dominance ratio of warm (0--60, 300--360 deg) vs cool (120--270 deg) hues |
...
| 0.4--0.7 | Functional color usage -- no major clashes but no strong palette design |
...
| 0.4--0.7 | Moderate detail -- functional rendering without exceptional complexity |
...
| Sub-property | Weight | What it measures |
| --- | --- | --- |
| Contrast ratio | 0.25 | Robust percentile contrast on V channel (95th -- 5th percentile) |
...
| Highlight/shadow balance | 0.15 | Tri-zone histogram analysis -- shadow, midtone, highlight proportions and their entropy |
...
| 0.4--0.7 | Adequate lighting -- functional but not distinctive |
...
| Subject scale | 0.15 | Subject area as fraction of frame, scored with preference for medium-to-closeup range (15--50%) |
...
| 0.4--0.7 | Moderate emphasis -- subject present but framing could be stronger |
...
| `closeup` | Subject 30--60% of frame | Close framing on subject |
| `medium_shot` | Subject 15--30% of frame | Standard character framing |
| `wide_shot` | Subject 5--15% of frame | Subject with environmental context |
...
- Subject scale scoring prefers the medium-to-closeup range (15--50% of frame). Extreme close-ups (>60%) and very wide shots (<5%) score lower.
...
Layer consistency measures how consistent the rendering is *within* each layer (foreground and background independently). Cross-layer differences are not penalized -- anime routinely uses different rendering approaches for characters and backgrounds.
...
| 0.4--0.7 | Typical range for most anime screenshots -- adequate quality and consistency |
...
| subject | `closeup` | Subject 30--60% of frame area |
| subject | `medium_shot` | Subject 15--30% of frame area |
| subject | `wide_shot` | Subject 5--15% of frame area |
